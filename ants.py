...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if isinstance(insect, QueenAnt) and insect.impostor_status == False:
                return
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        change = 1
        entr = self.place.entrance
        while True:
            if entr == hive:
                return None
            if entr.bees and change <= self.max_range and change >= self.min_range:
                return random_or_none(entr.bees)
            entr = entr.entrance
            change += 1
        # END Problem 5
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 4
        armor_after_reduce = self.armor - amount
        if armor_after_reduce <= 0:
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self, super_ants = []):
        # BEGIN Problem 13
        ScubaThrower.__init__(self)
        QueenAnt.queen_amount += 1
        self.super_ants = super_ants
        if QueenAnt.queen_amount > 1:
    ...
    def action(self, colony):
        ...
        if self.impostor_status == True:
            ...
            while exit != None:
                if exit.ant != None:
                    if exit.ant.container == True and exit.ant.ant != None:
                        if exit.ant.ant not in self.super_ants:
                            exit.ant.ant.damage *= 2
                            self.super_ants.append(exit.ant.ant)
                    else:
                        if exit.ant not in self.super_ants:
                            exit.ant.damage *= 2
                            self.super_ants.append(exit.ant)
                exit = exit.exit
    ...
    def reduce_armor(self, amount):
        ...
        self.armor -= amount
        self.place.remove_insect(self)
        if self.armor <= 0 and self.impostor_status == False:
            bees_win()
        # END Problem 13
